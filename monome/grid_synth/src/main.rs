...
// Snapshot everything that might explain why the audio callback
// stopped firing. Called from the main thread when the heartbeat
// has logged STALL twice in a row. Best-effort: missing tools or
// permissions just produce a partial dump. Output goes to
// /tmp/grid_synth-stall-<unix>-<reason>.txt.
fn capture_stall_diagnostics(reason: &str, audio_tid: u32) {
