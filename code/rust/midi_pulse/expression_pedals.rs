...
//! (`code/python/pedals/bridge.py`), which re-emits their MIDI on a virtual ALSA-seq port
//! named "MPC-20 pedals". This module connects to that port (midir) and tracks each pedal's
//! latest position. Background: `code/python/pedals/{README,bridge-plan}.org`.
//!
...
/// see `code/python/pedals/README.org` "Observed range"). The pedals' real range is ~1..120,
...
fn select_port(midi_in: &MidiInput, substring: &str) -> Result<MidiInputPort, String> {
  ...
  Err(format!(
    "no MIDI input matching {substring:?}; available: {available:?}. Is the bridge running? \
     (run code/python/pedals/bridge.py on the host)"
  ))
}
