...
use decode::{collect_control_changes, gain_from_intensity, DrumEvent, TetherDecoder};
...
/// intensity-scaled by THIS press's own `intensity` (0.0..1.0) -- for a `Ditto` pad, its
/// source pad's static `gain` trim scaled by the ditto press's own intensity, not the
...
fn resolve_fire(
  ...
  match kind {
    PadKind::Sample { sample, gain, .. } => {
      let resolved = gain * gain_from_intensity(intensity, db_range);
    ...
    PadKind::Ditto => {
      ...
      Some((Arc::clone(&hit.sample), hit.gain * gain_from_intensity(intensity, db_range)))
    }
  }
}
...
/// intensity. `None` only if `last` is empty (shouldn't happen mid-window; a Revise
...
fn resolve_revise(
  ...
  match kind {
    PadKind::Sample { gain, .. } => Some(gain * gain_from_intensity(intensity, db_range)),
    PadKind::Ditto => {
      let guard = last.lock().unwrap_or_else(|e| e.into_inner());
      let hit = guard.as_ref()?;
      Some(hit.gain * gain_from_intensity(intensity, db_range))
    }
  }
}

/// The poll loop for one device: every ~1 ms, ask the decoder for events. A `Fire` starts
/// a voice at `pad.gain * gain_from_intensity(intensity)`; a `Revise` ramps that pad's
/// current voice up to a higher intensity read later in the attack window.
fn run_voice_timer(
      ...
      match *event {
        DrumEvent::Fire { label, intensity } => {
          ...
          if let Some(binding) = &pad_map[slot] {
            match resolve_fire(&binding.kind, &binding.last, intensity, db_range) {
              Some((sample, gain)) => {
                if trace {
                  ...
                  eprintln!("[kmss]   pad {label} FIRE intensity {intensity:.3} -> {desc} (gain {gain:.3})");
                }
            ...
            eprintln!("[kmss]   pad {label} intensity {intensity:.3}: unmapped");
          }
        }
        DrumEvent::Revise { label, intensity } => {
          let slot = (label % 10) as usize;
          if let (Some(binding), Some(id)) = (&pad_map[slot], current[slot]) {
            if let Some(gain) = resolve_revise(&binding.kind, &binding.last, intensity, db_range) {
              if trace {
                eprintln!("[kmss]   pad {label} REVISE intensity {intensity:.3} -> gain {gain:.3}");
              }
...
mod tests {
  ...
  #[test]
  fn a_sample_fire_resolves_intensity_scaled_gain_and_records_its_static_trim() {
    ...
    assert!(gain < 0.8, "a sub-max-intensity hit resolves below the pad's base gain: {gain}");
    ...
    assert!(
      ...
      "records the pad's static gain TRIM (0.8), not the intensity-resolved gain ({gain}): got {}",
      hit.gain
    );
  }
  ...
  #[test]
  fn ditto_uses_the_source_pads_trim_but_its_own_presss_intensity() {
    let last: Mutex<Option<LastHit>> = Mutex::new(None);
    let kick = sample(2.0);
    ...
    // A max-intensity ditto press: same sample, gain = the KICK's 0.8 trim scaled by
    // the DITTO's own (max) intensity -- i.e. unity intensity factor -> 0.8 exactly.
    ...
    assert!((echoed_gain - 0.8).abs() < 1e-6, "intensity 1.0 against the kick's 0.8 trim = 0.8: {echoed_gain}");
  }
  ...
  fn a_hard_ditto_press_plays_louder_than_the_soft_hit_it_echoes() {
    ...
    // from the DITTO press's own (hard) intensity, not the original soft hit's.
    let last: Mutex<Option<LastHit>> = Mutex::new(None);
    let hat = sample(5.0);
    ...
    assert!((hard_ditto_gain - 1.0).abs() < 1e-6, "intensity 1.0 against the hat's 1.0 trim = unity: {hard_ditto_gain}");
  }
  ...
  fn revise_scales_a_sample_pad_and_a_ditto_pad_once_something_has_played() {
    ...
    assert!(scaled > 0.0 && scaled < 1.0, "a sub-max intensity revises below unity: {scaled}");
    ...
    // its own intensity's, not the original hit's, so it must correct the same way).
    ...
    assert!((ditto_scaled - 1.0).abs() < 1e-6, "intensity 1.0 against the hat's 1.0 trim = unity: {ditto_scaled}");
  }
}
