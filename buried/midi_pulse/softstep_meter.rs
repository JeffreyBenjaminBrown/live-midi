...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  while !quit.load(Ordering::SeqCst) {
    draw(&state, start, params, audio_on);
...
/// the two most-recently-struck pads, newest first, each as four sensor bars then their
/// sum bar, with the top pad's running hit-count. Then the last-hit line, the parameter
/// footer, and the recent-event log.
fn draw(state: &Mutex<MeterState>, start: Instant, params: SoftstepParams, audio_on: bool) {
  let now = Instant::now();
  ...
  if st.recent_pads.is_empty() {
    ...
    for (pos, rp) in recent.iter().enumerate() {
      ...
      for k in 0..4 {
        ...
        // `pv` is the peak-hold value -- the same recent max the `|` marker sits at.
